  ...
  methods: {
    ...
    async viewObservations(item) {
      try {
        ...
        // Fetch data from API with homevisitor parameter
        const homevisitor = item.homevisitor || ''
        const url = `/api/parenting2025_census/get/homevisit/sup/gethomevisit_dashboard.php?homevisitor=${encodeURIComponent(homevisitor)}`
        const response = await this.$axios.$get(url)
        
        if (response.ok && response.results && response.results.length > 0) {
          const visitorData = response.results[0]
          
          // Map classroomObsRecord to observations format
          const observations = []
          
          if (visitorData.classroomObsRecord && visitorData.classroomObsRecord.length > 0) {
            visitorData.classroomObsRecord.forEach((record, index) => {
              observations.push({
                childName: record.fullName || record.child_name || record.stid || '-',
                visitDate: record.date_obs || record.dateObs || '-',
                visitNumber: record.visit_number || record.visitNumber || index + 1,
                visitTime: record.time_obs || record.timeObs || '-',
                rawData: record
              })
            })
          }
          
          this.observationsForm.observations = observations
        }
      } catch (error) {
