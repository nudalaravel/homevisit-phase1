    ...
    <div class="table-container">
      ...
      >
        ...
        <template #cell(visitDate)="row">
          {{ formatThaiDate(row.item.visitDate) }}
        </template>
    ...
    >
      <div class="photo-view-content">
        <div class="patient-info-bar-small">
          ...
          <span class="badge badge-info">{{ formatThaiDate(photoModalData.visitDate) }}</span>
        </div>
    ...
    >
      <div class="correction-modal-content">
        <div class="correction-info" v-if="correctionItem">
          ...
          <p><strong>วันที่เยี่ยม:</strong> {{ formatThaiDate(correctionItem.visitDate) }}</p>
        </div>
...
export default {
  ...
  methods: {
    ...
    async submitCorrectionRequest() {
      ...
      try {
        // Mockup save - แสดงข้อมูลที่จะบันทึก
        const correctionData = {
          itemId: this.correctionItem.id || this.correctionItem.stid,
          childName: this.correctionItem.childName,
          visitorName: this.correctionItem.visitorName,
          visitDate: this.correctionItem.visitDate,
          reason: this.correctionReason.trim(),
          requestedAt: new Date().toISOString()
        }
        
        console.log('Correction Request Data (Mockup):', correctionData)
        
        // TODO: Implement actual API call here
        // await this.$axios.post('/api/.../request-correction', correctionData)
