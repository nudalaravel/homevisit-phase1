  ...
  <div class="supervisor-document">
    ...
    <div class="table-container">
      ...
      <div v-if="loadingMore" class="loading-more">
        ...
        <span class="loading-text">กำลังโหลดข้อมูลเพิ่มเติม...</span>
      </div>

      <!-- No More Data Indicator -->
      <div v-if="!loading && !loadingMore && !hasMoreData && tableData.length > 0" class="no-more-data">
        <span>แสดงข้อมูลทั้งหมดแล้ว</span>
      </div>
    </div>
...
export default {
  ...
  methods: {
      ...
      try {
        ...
        if (isSuccess && response.results && response.results.length > 0) {
          const newItems = response.results.map((item, index) => ({
            id: item.no || (this.tableData.length + index),
            no: item.no,
    ...
    viewReceipt(item) {
      // ใช้ข้อมูลจาก API (rawData) ถ้ามี
      const raw = item.rawData || {}
      
      this.receiptData = {
        visitorName: item.visitorName,
        idCard: item.idCard !== '-' ? item.idCard : (raw.PID || '-'),
        address: item.address !== '-' ? item.address : (raw.Address || '-'),
        paymentDate: item.paymentDate,
        formattedDate: this.formatReceiptDate(item.paymentDate),
        installment: item.installment,
        amount: item.amount,
        // เก็บข้อมูลเพิ่มเติมจาก API
    ...
    async toggleSendReceipt(item, event) {
      ...
      const username = this.$auth?.user?.username || 'unknown'
      
      try {
        // เรียก API บันทึก doc1, doc1_by, doc1_date
        const response = await this.$axios.$post('/api/parenting2025_census/get/homevisit/updatehomevisitpayment.php', {
          no: item.no,
          doc1: newValue ? '1' : '0',
          doc1_by: newValue ? username : null,
          doc1_date: newValue ? new Date().toISOString() : null
        })
        
        if (response.message === 'success') {
          item.sendReceipt = newValue
          item.doc1_by = newValue ? username : null
          item.doc1_date = newValue ? new Date().toISOString() : null
          this.$toast.success(
          ...
          this.$toast.error('ไม่สามารถบันทึกได้')
        }
      } catch (error) {
    ...
    async toggleSendIdCard(item, event) {
      ...
      try {
        // เรียก API บันทึก doc2, doc2_by, doc2_date
        const response = await this.$axios.$post('/api/parenting2025_census/get/homevisit/updatehomevisitpayment.php', {
          no: item.no,
          doc2: newValue ? '1' : '0',
          doc2_by: newValue ? username : null,
          doc2_date: newValue ? new Date().toISOString() : null
        })
        
        if (response.message === 'success') {
          item.sendIdCard = newValue
          item.doc2_by = newValue ? username : null
          item.doc2_date = newValue ? new Date().toISOString() : null
          this.$toast.success(
