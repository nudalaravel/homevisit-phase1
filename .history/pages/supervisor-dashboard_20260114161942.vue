  ...
  methods: {
    ...
    async fetchTableData() {
      ...
      try {
        ...
        if (!this.dataFetched) {
          // ใช้ API เดียวกับ supervisor-booking.vue
          const url = `/api/parenting2025_census/get/homevisit/sup/gethomevisit_result_data.php`
          console.log('[fetchTableData] URL:', url)
          const response = await this.$axios.$get(url)
          console.log('[fetchTableData] Response:', response)
          ...
          if (isSuccess && response.results) {
            ...
            // Group by stid เพื่อหาข้อมูลสรุปต่อเด็ก
            const childMap = {}
            response.results.forEach(item => {
              const stid = item.stid
              if (!stid) return
              
              const visitNumber = parseInt(item.time_visit) || 1
              ...
              if (!childMap[stid]) {
                childMap[stid] = {
                  ...
                  ampCode: item.tam_code ? item.tam_code.substring(0, 4) : '',
                  maxVisitNumber: visitNumber,
                  latestVisit: item,
                  visits: [item]
                }
              } else {
                childMap[stid].visits.push(item)
                // อัพเดทถ้าเป็นการเยี่ยมครั้งใหม่กว่า
                if (visitNumber > childMap[stid].maxVisitNumber) {
                  childMap[stid].maxVisitNumber = visitNumber
                  childMap[stid].latestVisit = item
                }
              }
            })
            
            // แปลงเป็น array สำหรับแสดงในตาราง
            this.rawTableData = Object.values(childMap).map((child, index) => ({
              id: `${child.stid}-${index}`,
              stid: child.stid,
              recby: child.recby,
              ampCode: child.ampCode,
              visitorName: child.visitorName,
              childName: child.childName,
              lastVisitDate: child.latestVisit.date_visit || '-',
              lastVisitTime: child.latestVisit.timeStart || '-',
              visitNumber: child.maxVisitNumber,
              nextVisitDate: child.latestVisit.q10_appDate || '-',
              nextVisitTime: child.latestVisit.q10_appTime || '-',
              postponeCount: parseInt(child.latestVisit.cnt_app) || 0,
              isPostponed: (parseInt(child.latestVisit.cnt_app) || 0) > 0,
              recStatus: child.latestVisit.recStatus,
              tamCode: child.tamCode,
              prefix: child.prefix,
              totalVisits: child.visits.length
            }))
            console.log('[fetchTableData] Grouped data:', this.rawTableData)
    ...
    filterTableData() {
      ...
      // *** ต้องมีบรรทัดนี้เพื่อ assign ข้อมูลไปแสดงในตาราง ***
      this.tableData = filtered
      console.log('[filterTableData] tableData:', this.tableData.length, 'items')
    },
