        ...
        if (response.statusCode === 200 && response.results) {
          this.visitorOptions = [
            ...
            ...response.results.map(item => ({
              ...
              text: item.name || item.fullname || `${item.fname} ${item.lname}`
            }))
          ]
        }
      } catch (error) {
      ...
      try {
        ...
        if (response.statusCode === 200 && response.results) {
          this.tableData = response.results.map(item => ({
            id: item.id,
            stid: item.stid,
            visitorName: item.visitor_name || item.visitorName || '-',
            childName: item.child_name || item.childName || item.name || '-',
            lastVisitDate: item.last_visit_date || item.lastVisitDate || '-',
            lastVisitTime: item.last_visit_time || item.lastVisitTime || '-',
            visitNumber: item.visit_number || item.visitNumber || 0,
            nextVisitDate: item.next_visit_date || item.nextVisitDate || '-',
            nextVisitTime: item.next_visit_time || item.nextVisitTime || '-',
            postponeCount: item.postpone_count || item.postponeCount || 0,
            isPostponed: (item.postpone_count || item.postponeCount || 0) > 0
          }))
        } else {
          this.tableData = []
        }
