        ...
        if (isSuccess && response.results) {
          ...
          this.visitorOptions = [
            ...
            ...homeVisitors.map(item => ({
              ...
              // แสดงชื่อ-นามสกุล พร้อมจังหวัด
              text: `${item.fname || ''} ${item.lname || ''}`.trim() + 
                    (item.prov_nameT ? ` (${item.prov_nameT})` : '')
            }))
          ]
        }
      } catch (error) {
