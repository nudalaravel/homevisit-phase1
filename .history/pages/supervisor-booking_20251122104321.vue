...
export default {
  ...
  data() {
    return {
      ...
      tableData: [
        {
          visitDate: 'อ. 20 มิถุนายน 2566 (27)',
          visitorName: 'น.ส.ลิเยาะ กาลาแต',
          childName: 'วายุฤทธิ์ ช่วยสุข',
          recordStatus: 'completed',
          hasPhotos: true,
          confirmStatus: 'confirmed',
          surveyId: null // Will be populated from actual data
        },
        {
          visitDate: 'จ. 17 กรกฎาคม 2566 (31)',
          visitorName: 'ณณัชชา จิตรสุวรรณ์',
          childName: 'กุลนดา เกตุด้วง',
          recordStatus: 'completed',
          hasPhotos: true,
        ...
        {
          visitDate: 'ส. 22 กรกฎาคม 2566 (31)',
          visitorName: 'ทัศนีย์ เกตุด้วง',
          childName: 'ธนวัฒน์ คงเขียว',
        ...
        {
          visitDate: 'จ. 24 กรกฎาคม 2566 (32)',
          visitorName: 'ณณัชชา จิตรสุวรรณ์',
          childName: 'กาญจนา จันนา',
          recordStatus: 'pending',
          hasPhotos: false,
          confirmStatus: 'pending',
          surveyId: null
        ...
        {
          visitDate: 'อ. 25 กรกฎาคม 2566 (32)',
          visitorName: 'ณณัชชา จิตรสุวรรณ์',
          childName: 'กุลนดา เกตุด้วง',
          recordStatus: 'pending',
          hasPhotos: true,
        ...
        {
          visitDate: 'ส. 05 สิงหาคม 2566 (35)',
          visitorName: 'ธนวรรณ ชูดำ',
          childName: 'ธนวิชญ์ เกื้อคลัง',
          recordStatus: 'completed',
          hasPhotos: true,
          confirmStatus: 'warning',
          surveyId: null
  ...
  methods: {
    async viewRecord(item) {
      try {
        ...
        // If surveyId is available, load from IndexedDB
        if (item.surveyId) {
          const survey = await this.$indexedDB.getSurveyProgressById(item.surveyId)
          if (survey) {
            await this.loadRecordData(survey, item)
          } else {
            this.$toast.warning('ไม่พบข้อมูลการเยี่ยมบ้าน')
            this.recordData = null
          }
        } else {
          // For mock data, use item data
          const mockActivities = this.generateMockActivities([], 44, 2)
          this.recordData = {
            childName: item.childName,
            visitorName: item.visitorName,
            visitDate: item.visitDate,
            visitNumber: this.extractVisitNumber(item.visitDate),
            startTime: '16:00 น.',
            monthAge: 44,
            time: 2,
            answers: {
    ...
    async viewPhotos(item) {
      try {
        ...
        // If surveyId is available, load images from survey
        if (item.surveyId) {
          const survey = await this.$indexedDB.getSurveyProgressById(item.surveyId)
          if (survey) {
