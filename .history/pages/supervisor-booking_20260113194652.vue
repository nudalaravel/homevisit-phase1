    ...
    >
      <div v-if="recordData" class="visit-record-wrapper">
        ...
        <div v-if="recordData.q9Activities && recordData.q9Activities.length > 0" class="record-section">
          <div class="section-title">กิจกรรมการเยี่ยมบ้านครั้งนี้ (ข้อ 9)</div>
          <table class="activity-table">
            <thead>
              <tr>
                <th>รหัสกิจกรรม</th>
        ...
        <div class="record-section">
          ...
          <div class="question-item">
            <div class="question-text">ใครทำกิจกรรมกับเด็ก (ข้อ 6)</div>
            <div class="answer-text">{{ getQ6Answer(recordData.answers?.q6) }}</div>
          </div>
          <div class="question-item">
            <div class="question-text">มีผู้อื่นร่วมทำกิจกรรมด้วยหรือไม่ (ข้อ 7)</div>
            <div class="answer-text">{{ recordData.answers?.q7 === 1 ? 'มี' : recordData.answers?.q7 === 3 ? 'ไม่มี' : '-' }}</div>
          </div>
          <div v-if="recordData.answers?.q71" class="question-item">
            <div class="question-text">ผู้ร่วมทำกิจกรรม (ข้อ 7.1)</div>
            <div class="answer-text">{{ recordData.answers?.q71 }}</div>
          </div>
          <div class="question-item">
            <div class="question-text">มีเด็กคนอื่นร่วมทำกิจกรรมด้วยหรือไม่ (ข้อ 8)</div>
            <div class="answer-text">{{ recordData.answers?.q8 === 1 ? 'มี' : recordData.answers?.q8 === 3 ? 'ไม่มี' : '-' }}</div>
          </div>
          ...
          <div class="question-item">
            <div class="question-text">เวลาสิ้นสุดการเยี่ยมบ้าน</div>
            <div class="answer-text">{{ recordData.endTime || '-' }}</div>
          </div>
        </div>

        <!-- Appointment Section -->
        <div v-if="recordData.appointment && (recordData.appointment.date || recordData.appointment.time)" class="record-section">
          <div class="section-title">นัดหมายการเยี่ยมบ้านครั้งถัดไป (ข้อ 10)</div>
          <div class="question-item">
            <div class="question-text">วันที่นัดหมาย</div>
            <div class="answer-text">{{ recordData.appointment.date || '-' }}</div>
          </div>
          <div class="question-item">
            <div class="question-text">เวลานัดหมาย</div>
            <div class="answer-text">{{ recordData.appointment.time || '-' }}</div>
          </div>
        </div>

        <!-- Photos Section -->
        <div v-if="recordData.pics && (recordData.pics.pic1 || recordData.pics.pic2)" class="record-section">
          <div class="section-title">รูปภาพ</div>
          <div class="photos-grid">
            <div v-if="recordData.pics.pic1" class="photo-item">
              <div class="photo-label">รูปของเล่น/สื่ออุปกรณ์</div>
              <img :src="recordData.pics.pic1" alt="รูปของเล่น" class="photo-image" @click="openImageViewer(recordData.pics.pic1)" />
            </div>
            <div v-if="recordData.pics.pic2" class="photo-item">
              <div class="photo-label">รูปเด็กและผู้ปกครองทำกิจกรรม</div>
              <img :src="recordData.pics.pic2" alt="รูปกิจกรรม" class="photo-image" @click="openImageViewer(recordData.pics.pic2)" />
            </div>
          </div>
        </div>
        </div>
    ...
    async viewRecord(item) {
      try {
        ...
        if (item.rawData) {
          ...
          const q9Activities = []
          for (let i = 1; i <= 5; i++) {
            const activityName = raw[`q9${i}_name`]
            const activityAnswer = raw[`q9${i}`]
            if (activityName) {
              q9Activities.push({
                no: activityName,
                title: `กิจกรรม ${activityName}`,
          ...
          const q5Activities = []
          for (let i = 1; i <= 5; i++) {
            const activityName = raw[`q5${i}_name`]
