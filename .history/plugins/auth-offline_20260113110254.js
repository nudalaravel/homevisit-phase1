export default function ({ app, store }, inject) {
  ...
  class OfflineAuthManager {
    ...
    setupAuthListeners() {
      ...
      const checkAuthModule = () => {
        if (store.state.auth) {
          ...
          store.watch(
            ...
            (loggedIn, oldLoggedIn) => {
              ...
              if (loggedIn && !oldLoggedIn) {
                // รอจนกว่า user data จะพร้อมก่อน save (retry mechanism)
                this.waitForUserDataAndSave();
          ...
          // Watch user data changes to ensure level is preserved
          store.watch(
            (state) => state.auth.user,
            (newUser, oldUser) => {
              if (newUser && newUser !== oldUser) {
                // Check if user level is missing but should be there
                if (newUser.level === undefined && process.client) {
                  ...
                  try {
                    ...
                    if (authDataStr) {
                      ...
                      if (authData && authData.user && authData.user.level !== undefined) {
                        console.log("OfflineAuth: Restoring user level from localStorage:", authData.user.level);
                        // Update user with level
                        store.commit("auth/SET", ["user", { ...newUser, level: authData.user.level }]);
                      }
                    }
                  } catch (e) {
                    console.error("OfflineAuth: Error restoring user level:", e);
                  }
                }
              }
            },
    ...
    // รอจนกว่า user data จะพร้อมแล้วค่อย save (retry mechanism)
    waitForUserDataAndSave(attempt = 1, maxAttempts = 20) {
      // รอ 150ms ก่อนตรวจสอบ (ให้ API มีเวลาตอบกลับ)
      setTimeout(() => {
        const userData = store.state.auth?.user;
        const hasUserData = userData && Object.keys(userData).length > 0;

        if (hasUserData) {
          // User data พร้อมแล้ว - save ได้เลย
          this.saveAuthData();
        } else if (attempt < maxAttempts) {
          // ยังไม่พร้อม - ลองใหม่
          this.waitForUserDataAndSave(attempt + 1, maxAttempts);
        } else {
          // ถึง max attempts แล้ว - ลอง save จาก localStorage ที่มีอยู่เดิม
          const existingAuth = this.loadAuthData();
          if (existingAuth && existingAuth.user) {
            console.log("OfflineAuth: Using existing auth data from localStorage");
          } else {
            console.warn("OfflineAuth: Max attempts reached, user data still not available. Will retry on next login.");
          }
        }
      }, 150);
    }
    ...
    saveAuthData() {
      try {
        ...
        if (!plainUser || Object.keys(plainUser).length === 0) {
          console.warn("OfflineAuth: saveAuthData - user data is empty");
          return;
        }

        // Debug: Log user data before saving
        console.log("OfflineAuth: saveAuthData - Saving user data:", {
          username: plainUser.username,
          level: plainUser.level,
          hasLevel: plainUser.level !== undefined,
          userKeys: Object.keys(plainUser)
        });
