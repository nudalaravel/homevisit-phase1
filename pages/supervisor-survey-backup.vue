...
export default {
  ...
  data() {
    return {
      ...
      allVisitResults: [], // ข้อมูลทั้งหมดจาก gethomevisit_result.php
      q3Options: [
        { value: 1, label: 'แม่ (1)' },
        { value: 2, label: 'พ่อ (2)' },
        { value: 3, label: 'ย่า (3)' },
  ...
  computed: {
    ...
    childOptions() {
      ...
      // Filter visit results by selected visitor's username
      const filteredResults = this.allVisitResults.filter(
        item => item.recby === this.form.visitor
      )
      
      // Create unique child options (stid as value, fullname_visit as text)
    ...
    // Filter visit dates by selected child (stid)
    visitDateOptions() {
      const options = [{ value: null, text: '--เลือก--' }]
      ...
      // Filter visit results by selected child's stid
      const filteredResults = this.allVisitResults.filter(
        item => item.stid === this.form.child && item.recby === this.form.visitor
      )
      
      // Create visit date options from filtered results
      filteredResults.forEach(item => {
        if (item.date_visit) {
          const formattedDate = this.formatThaiDateOption(item.date_visit)
          options.push({
            value: `${item.stid}_${item.time_visit}`, // Unique value combining stid and time_visit
  ...
  methods: {
    ...
    // Fetch visitors from getuser.php
    async fetchVisitors() {
      try {
    ...
    // Fetch visit results from gethomevisit_result.php
    ...
    // Format date to Thai format for dropdown option
    ...
    resetChildSelection() {
      ...
      // Reinitialize Select2 for child dropdown
      this.$nextTick(() => {
    ...
    resetVisitDateSelection() {
      ...
      // Reinitialize Select2 for visitDate dropdown
      this.$nextTick(() => {
        if (this.$select2 && this.$refs.visitDateSelect && window.$) {
          window.$(this.$refs.visitDateSelect).val(null).trigger('change')
        }
  ...
  watch: {
    ...
    // Watch computed childOptions to reinitialize Select2
    childOptions: {
      handler() {
        this.$nextTick(() => {
          if (this.$select2 && this.$refs.childSelect && window.$) {
            // Destroy and reinitialize to update options
    ...
    // Watch computed visitDateOptions to reinitialize Select2
    visitDateOptions: {
      handler() {
        this.$nextTick(() => {
          if (this.$select2 && this.$refs.visitDateSelect && window.$) {
